...
import { useFacilities, useCreateFacility, useUpdateFacility, useDeleteFacility } from '@/hooks/resources'
import { IconPicker } from '@/components/ui/icon-picker'
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select'
import { useState } from 'react'
import { Loader2, Plus, Home, Edit2, Trash2, X, Search, ChevronLeft, ChevronRight } from 'lucide-react'
...
export default function FacilitiesPage() {
    ...
    const { data, isLoading } = useFacilities({ keyword: debouncedSearch, page, limit: 20 })
    ...
    const resetForm = () => {
        setForm({ name: '', icon: 'Home', description: '', category: '' })
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingFacility) {
            updateFacility.mutate(
                { id: editingFacility.id, data: form },
                { onSuccess: resetForm }
            )
        } else {
            createFacility.mutate(form, { onSuccess: resetForm })
        }
    }

    const startEdit = (facility: any) => {
    ...
    const handleDelete = (id: string, name: string) => {
        if (confirm(`Delete facility "${name}"? This will remove it from all homestays.`)) {
            ...
            <div className="max-w-8xl mx-auto px-4 sm:px-6 md:px-8 lg:px-0 space-y-8">
                ...
                <div className="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p style={{ fontFamily: 'var(--font-mona-sans), sans-serif' }} className="text-gray-600 mt-2">
                            Manage global facilities for homestays
                        </p>
                    </div>
                    {!isFormOpen && (
                        <button
                ...
                {isFormOpen && (
                    <form onSubmit={handleSubmit} className="bg-white rounded-lg border border-gray-200 p-6 space-y-4">
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-lg font-semibold">{editingFacility ? 'Edit Facility' : 'Create New Facility'}</h2>
                            <button
                                type="button"
                                onClick={resetForm}
                        ...
                        <div className="flex gap-2 pt-2">
                            ...
                            >
                                {(createFacility.isPending || updateFacility.isPending) && <Loader2 size={16} className="animate-spin" />}
                ...
                {!isLoading && facilities.length === 0 && (
                    <div className="text-center py-12 bg-white rounded-lg border border-gray-200">
                        ...
                        <p className="text-gray-600 mb-4">Create your first facility to add amenities to homestays.</p>
                        <button
                            onClick={() => setIsFormOpen(true)}
                    ...
                    <>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                            {facilities.map((facility: any) => {
                                const IconComponent = require('lucide-react')[facility.icon] || Home
                                return (
                                    <div
                                        key={facility.id}
                                    ...
                                    >
                                        <div className="flex items-start gap-3">
                                            <div className="w-10 h-10 rounded-lg bg-[#005246]/10 text-[#005246] flex items-center justify-center">
                                                <IconComponent size={20} />
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <h3 className="font-semibold text-gray-900 truncate">{facility.name}</h3>
                                                {facility.category && (
                                                    <p className="text-xs text-gray-500 mt-0.5">{facility.category}</p>
                                                )}
                                            </div>
                                        </div>
                        ...
                        {meta && meta.totalPages > 1 && (
                            <div className="flex items-center justify-between bg-white rounded-lg border border-gray-200 px-4 py-3">
                                <div className="text-sm text-gray-600">
                                    Showing {((meta.page - 1) * meta.limit) + 1} to {Math.min(meta.page * meta.limit, meta.total)} of {meta.total} facilities
                                </div>
