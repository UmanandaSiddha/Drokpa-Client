...
function ProviderOverview() {
    ...
    const links: { label: string; href: string; desc: string }[] = [
        ...(isHost ? [{ label: 'My Homestays', href: '/admin/my-listings/homestays', desc: 'View and manage your homestay listings and rooms.' }] : []),
        ...(isVendor ? [{ label: 'My Vehicles', href: '/admin/my-listings/vehicles', desc: 'Manage your vehicle fleet and availability.' }] : []),
        ...(isGuide ? [{ label: 'My Guide Profile', href: '/admin/my-listings/guide', desc: 'Update your guide profile and languages.' }] : []),
        { label: 'My Bookings', href: '/admin/bookings', desc: 'See all bookings for your services.' },
        { label: 'Payouts', href: '/admin/payouts', desc: 'Track your earnings and payout history.' },
    ]
            ...
            <div className="admin-stat-grid">
                {links.map((l) => (
                    <Link key={l.href} href={l.href} style={{ textDecoration: 'none' }}>
                        <div className="admin-stat-card admin-stat-card--green" style={{ cursor: 'pointer', flexDirection: 'column', alignItems: 'flex-start', gap: '0.5rem' }}>
                            <p className="admin-stat-card__label" style={{ marginBottom: 0 }}>{l.label}</p>
                            <p style={{ fontSize: '0.8rem', color: '#64748b', margin: 0 }}>{l.desc}</p>
                            <ArrowRight size={16} style={{ color: '#005246', marginTop: '0.25rem' }} />
                        </div>
                    </Link>
                ))}
            </div>
